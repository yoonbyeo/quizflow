...
import {
  ArrowLeft, Edit3, BookOpen, Brain, Shuffle, Target, RotateCcw,
  ChevronLeft, ChevronRight, Clock, TrendingUp, CheckCircle, XCircle
} from 'lucide-react';
...
export default function SetDetailPage() {
  ...
  const { cardSets, resetStats } = useStore();
  ...
  const cardStats = Object.values(set.studyStats.cardStats);
  ...
  const modes = [
    { label: '플래시카드', desc: '카드를 넘기며 암기', icon: BookOpen, path: `/study/${id}/flashcard`, color: 'from-blue-600 to-blue-700', glow: 'shadow-blue-500/30' },
    { label: '테스트', desc: '객관식 & 주관식 문제', icon: Brain, path: `/study/${id}/test`, color: 'from-violet-600 to-violet-700', glow: 'shadow-violet-500/30' },
    { label: '매칭', desc: '용어와 정의 매칭', icon: Shuffle, path: `/study/${id}/match`, color: 'from-emerald-600 to-emerald-700', glow: 'shadow-emerald-500/30' },
    { label: '쓰기', desc: '직접 입력해서 학습', icon: Target, path: `/study/${id}/write`, color: 'from-orange-600 to-orange-700', glow: 'shadow-orange-500/30' },
  ];
    ...
    <div className="min-h-screen pt-20 pb-12">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-start gap-4 mb-8">
          <button
            onClick={() => navigate('/')}
            className="p-2 rounded-xl text-slate-400 hover:text-slate-200 hover:bg-white/[0.08] transition-colors mt-1"
          >
            ...
            <div className="flex items-center gap-4 mt-2 text-sm text-slate-500">
              ...
              {set.studyStats.lastStudied && (
                <span className="flex items-center gap-1.5">
                  ...
                  마지막 학습: {formatDate(set.studyStats.lastStudied)}
                </span>
              )}
              {accuracy !== null && (
                <span className={cn(
          ...
          <Link
            to={`/edit/${id}`}
            className="flex items-center gap-2 px-4 py-2 rounded-xl text-slate-400 hover:text-slate-200 hover:bg-white/[0.08] transition-colors text-sm font-medium"
          >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
          {modes.map(({ label, desc, icon: Icon, path, color, glow }) => (
            <Link
              key={label}
              to={set.cards.length === 0 ? '#' : path}
              className={cn(
                'relative overflow-hidden glass rounded-2xl p-5 group transition-all card-glow',
                set.cards.length === 0 ? 'opacity-50 cursor-not-allowed' : `hover:shadow-lg hover:${glow} card-glow-hover`
              )}
            ...
            >
              ...
              <div className="font-semibold text-slate-100 text-sm mb-0.5 group-hover:text-white transition-colors">
                {label}
              </div>
                    ...
                    return (
                      <div key={card.id} className="glass rounded-xl p-4 flex items-start gap-4 card-glow">
                        ...
                        <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-2">
                          <div>
                            <p className="text-xs text-slate-500 mb-0.5">용어</p>
                            <p className="text-sm font-medium text-slate-200">{card.term}</p>
                          </div>
                          <div>
                            <p className="text-xs text-slate-500 mb-0.5">정의</p>
                        ...
                        {stat && (
                          <div className="flex flex-col items-end gap-1 flex-shrink-0">
                            <span className={cn(
                              ...
                              stat.difficulty === 'hard' ? 'bg-red-500/15 text-red-400' :
                              'bg-slate-500/15 text-slate-500'
                            )}>
                              {stat.difficulty === 'easy' ? '쉬움' :
                               stat.difficulty === 'medium' ? '보통' :
                               stat.difficulty === 'hard' ? '어려움' : '미평가'}
                            </span>
            ...
            {totalAttempts === 0 ? (
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  {[
                    { label: '총 시도', value: totalAttempts, color: 'text-blue-400' },
                    { label: '정답', value: totalCorrect, color: 'text-emerald-400' },
                    { label: '오답', value: totalAttempts - totalCorrect, color: 'text-red-400' },
      ...
      <Modal open={resetConfirm} onClose={() => setResetConfirm(false)} title="통계 초기화" size="sm">
        ...
        <div className="flex gap-3 justify-end">
          ...
          <Button variant="danger" onClick={() => { resetStats(id!); setResetConfirm(false); }}>초기화</Button>
        </div>
      </Modal>
    </div>
  );
