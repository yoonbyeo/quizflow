...
export default function TestPage() {
  ...
  const generateQuestions = () => {
    ...
    for (const card of selected) {
      ...
      let mode: 'term' | 'definition' = config.questionMode === 'mixed'
  ...
  const handleSubmit = () => {
    ...
    updateCardStat(id!, currentQ.cardId, correct);
          ...
          <div className="glass rounded-2xl p-6 card-glow space-y-6">
            <div>
              ...
              <div className="flex justify-between text-xs text-slate-600 mt-1">
                <span>1</span><span>{set.cards.length}</span>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex flex-col gap-2">
                {[
                  { key: 'multiple-choice' as const, label: '객관식', desc: '4지선다형' },
                  { key: 'written' as const, label: '주관식', desc: '직접 입력' },
                ].map(({ key, label, desc }) => (
                  <button
                    key={key}
                    onClick={() => {
                      setConfig((c) => {
                        const types = c.questionTypes.includes(key)
                    ...
                    className={cn(
                      ...
                      config.questionTypes.includes(key)
            ...
            <div>
              ...
              <div className="flex gap-2">
                {[
                  { key: 'definition' as const, label: '용어 → 정의' },
                  { key: 'term' as const, label: '정의 → 용어' },
                  { key: 'mixed' as const, label: '혼합' },
                ].map(({ key, label }) => (
                  <button
                    key={key}
                    onClick={() => setConfig((c) => ({ ...c, questionMode: key }))}
                    className={cn(
                      'flex-1 py-2.5 rounded-xl text-xs font-medium border transition-all',
                      config.questionMode === key
                        ? 'border-blue-500/50 bg-blue-500/10 text-blue-400'
                        : 'border-slate-700 bg-slate-800/40 text-slate-400 hover:border-slate-600'
                    )}
                  >
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => setConfig((c) => ({ ...c, shuffleCards: !c.shuffleCards }))}
                className={cn(
                  'relative w-11 h-6 rounded-full transition-all',
                  config.shuffleCards ? 'bg-blue-500' : 'bg-slate-700'
                )}
              >
                <div className={cn(
                  'absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform',
                  config.shuffleCards && 'translate-x-5'
                )} />
              </button>
            </div>
            ...
            {questions.map((q, i) => {
              ...
              return (
                <div key={q.id} className={cn(
                  'glass rounded-xl p-4 flex items-start gap-3 border',
                  ans.correct ? 'border-emerald-500/20' : 'border-red-500/20'
                )}>
                  {ans.correct
    ...
    <div className="min-h-screen pt-20 pb-12">
      <div className="max-w-2xl mx-auto px-4 sm:px-6">
        ...
        <div className="glass rounded-2xl p-6 card-glow mb-4">
          ...
          <h2 className="text-lg font-semibold text-slate-100 leading-relaxed whitespace-pre-line mb-6">
            {currentQ?.question}
          </h2>
              ...
              {currentQ.options?.map((opt) => {
                ...
                return (
                  <button
                    ...
                    disabled={isAnswered}
                    className={cn(
                      'w-full text-left px-4 py-3.5 rounded-xl border font-medium text-sm transition-all',
                      style
                    )}
                  >
                    {opt}
        ...
        <div className="flex justify-end gap-3">
          {!isAnswered ? (
            <Button
              ...
              disabled={currentQ?.type === 'multiple-choice' ? !selectedAnswer : !writtenAnswer.trim()}
            >
              제출
        ...
        <p className="text-center text-xs text-slate-600 mt-3">Enter로 제출 · 다음 문제 이동</p>
      </div>
    </div>
  );
}
