    ...
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className={cn('px-3 py-1.5 rounded-lg text-xs font-medium transition-all', showDefinitionFirst ? 'bg-violet-500/20 text-violet-400' : 'text-slate-500 hover:text-slate-300 hover:bg-white/[0.06]')}
          >
            <Eye className="w-3.5 h-3.5 inline mr-1" />뜻 먼저
          </button>
          <button onClick={handleShuffle} className={cn('p-2 rounded-lg transition-all', isShuffled ? 'text-blue-400 bg-blue-500/15' : 'text-slate-500 hover:text-slate-300 hover:bg-white/[0.06]')}>
            <Shuffle className="w-4 h-4" />
          </button>
          <button onClick={handleReset} className="p-2 rounded-lg text-slate-500 hover:text-slate-300 hover:bg-white/[0.06] transition-all">
      ...
      <div className="flex items-center gap-3 mb-4">
        <div className="flex-1 h-1.5 bg-slate-800 rounded-full overflow-hidden">
          <div className="h-full rounded-full bg-gradient-to-r from-blue-500 to-violet-500 transition-all" style={{ width: `${progress * 100}%` }} />
        </div>
        <span className="text-sm text-slate-400 font-medium tabular-nums">{currentIndex + 1} / {cards.length}</span>
      </div>

      <div className="flex gap-4 mb-4 text-sm">
        <span className="flex items-center gap-1.5 text-emerald-400"><ThumbsUp className="w-3.5 h-3.5" /> {knownCount}</span>
        <span className="flex items-center gap-1.5 text-red-400"><ThumbsDown className="w-3.5 h-3.5" /> {unknownCount}</span>
      </div>
      ...
      {isFlipped && (
        <div className="flex gap-3 justify-center mb-6">
          <button onClick={() => handleRate(false)} className="flex items-center gap-2 px-6 py-3 rounded-2xl bg-red-500/15 border border-red-500/30 text-red-400 font-medium hover:bg-red-500/25 transition-all hover:scale-[1.02] active:scale-95">
            <ThumbsDown className="w-5 h-5" /> 모르겠어요
          </button>
          <button onClick={() => handleRate(true)} className="flex items-center gap-2 px-6 py-3 rounded-2xl bg-emerald-500/15 border border-emerald-500/30 text-emerald-400 font-medium hover:bg-emerald-500/25 transition-all hover:scale-[1.02] active:scale-95">
            <ThumbsUp className="w-5 h-5" /> 알아요
          </button>
        </div>
      )}

      <div className="flex items-center justify-center gap-4">
        <button onClick={goPrev} disabled={currentIndex === 0} className="p-3 rounded-2xl glass text-slate-400 hover:text-slate-200 disabled:opacity-30 transition-all hover:bg-white/[0.08]">
          <ChevronLeft className="w-6 h-6" />
        </button>
        <button onClick={goNext} className="flex items-center gap-2 px-6 py-3 rounded-2xl gradient-primary text-white font-medium shadow-lg shadow-blue-500/25 hover:shadow-blue-500/40 hover:scale-[1.02] transition-all">
